...
export default function Editor(){
  ...
  async function submit(e){
    ...
    try{
      ...
      if(!description.trim()) newErrors.push('Short description is required')
      if(!body.trim() || body.trim().length < 20) newErrors.push('Body must be at least 20 characters (use markdown)')
  ...
  return (
    <div className="container" style={{maxWidth:720}}>
      ...
      <form className="form" onSubmit={submit}>
        {errors.length>0 && <div style={{color:"red",marginBottom:8}}><ul>{errors.map((e,i)=>(<li key={i}>{e}</li>))}</ul></div>}
