...
import EditArticlefrom './components/EditArticle'
  ...
  return (
    <Layout style={{minHeight:'100vh'}}>
      <Header style={{display:'flex', alignItems:'center', justifyContent:'space-between'}}>
        <div style={{color:'#5fb46d', fontWeight:700, fontSize: '20px', marginLeft: 430}}>Realworld Blog</div>
        <div>
          <Link to="/" style={{marginRight:30, color:'#fff'}}>Home</Link>
          {auth && <Link to="/new" style={{marginRight:25, color:'#fff'}}>New post</Link>}
          {auth && <Link to="/settings" style={{marginRight:20, color:'#fff'}}>Settings</Link>}
          {auth ? (
            <>
              <Link to={ getUsername() ? ('/profile/' + getUsername()) : '/profile' } style={{marginRight:25, color:'#fff'}}>
